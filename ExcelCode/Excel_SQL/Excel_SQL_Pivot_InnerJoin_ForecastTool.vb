Private Function FN_Write_SQL_Pivot(dWeekDay As Date, _
   ...
   '*** Calculate the column to reference for weekly hours ***
    ...
    'Calculate Potential Hours
     lPotential_Hours = 40 * lNumberOfWeeks
    ...
    '** February 27th column data can't function with an greater than or less than sign
     If sExcel_ColumnRef = "F39" Or sExcel_ColumnRef = "F50" Then
    ...
    '       NOTE:   1) Logged Hours are equal same as  Hours per Week
    '               2) Potential Hours are a 40 hour week work week
    '               3) Activity Hours are Logged Hours minus admin, meeting, training and PTO
    
     sSQL = sSQL & ", " & sCalc_Hours & " as [Hours per Week]" & vbCrLf
     sSQL = sSQL & ", " & sCalc_Hours & " as [Logged Hours]" & vbCrLf
     sSQL = sSQL & ", " & lPotential_Hours & " as [Potential Hours]" & vbCrLf
     
    'Activity Hours: If Hour Category = Deal Hours , then include hour =sCalc_Hours , else don't =0
     sSQL = sSQL & ", IIF(tblOpp.[F35]=5," & sCalc_Hours & ",0) as [Activity Hours]" & vbCrLf
        

    'Utilizaton Hours:  If Hour Category = Deal Hours , then include hour =sCalc_Hours , else don't =0
    '                   PLUS If Hour Category = Practice Mgmt , then include hour =sCalc_Hours , else don't =0
     sSQL = sSQL & ", IIF(tblOpp.[F35]=5," & sCalc_Hours & ",0) + IIF(tblOpp.[F35]=13," & sCalc_Hours & ",0) as [Utilization Hours]" & vbCrLf
     
     
    'Deals Works: If Hour Category = Deal Hours , then include hour =sCalc_Hours , else don't =0
     sSQL = sSQL & ", IIF(tblOpp.[F35]=5,1,0) as [Deals Worked]" & vbCrLf
    ...
    '*** Opportunity Fields ***
     ...
     sSQL = sSQL & ", Trim(tblOpp.[F3]) as [Opp Name]" & vbCrLf
    ...
    'Solution Directors and  Bid Manager Opportunities
     sSQL = sSQL & ", tblEmp_SolutionDir.[F2] as [Solution Dir Full Name]" & vbCrLf
     sSQL = sSQL & ", tblEmp_SolutionDir.[F3] as [Solution Dir Last Name]" & vbCrLf
     sSQL = sSQL & ", tblEmp_SolutionDir.[F14] AS [Solution Dir Scott Archer Report Filter]" & vbCrLf
     sSQL = sSQL & ", tblEmp_BidMgr.[F2] as [Bid Manager Full Name]" & vbCrLf
...
'    sSQL = sSQL & ", tblOpp.[F29] as [Opp Status NextStep]" & vbCrLf
    ...
    'Forecast Line Item Removed and Removed Date
     ...
     sSQL = sSQL & ", tblEmpOpp.[F78] as [Removed Forecast ID]" & vbCrLf
   ...
   ' sSQL = sSQL & ") as tblPivotHours" & vbCrLf
    ...
    '--------------------------------------------------------------------------------------
    '** February 27th column data or May 15th column data can't function with an greater than or less than sign
     If sExcel_ColumnRef = "F39" Or sExcel_ColumnRef = "F50" Then
...
'     sSQL = sSQL & " WHERE tblPivotHours.[OppID]= 29 ;"
    ...
    '
     ...
     sSQL = sSQL & " ,tblPivotHours.[Client Name] ,tblPivotHours.[Status], tblPivotHours.[Opp Name]"
